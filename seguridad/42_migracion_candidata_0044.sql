-- ============================================================================
-- 0044_post_review_abuse_and_visibility_hardening.sql (CANDIDATA — NO_EJECUTAR_SIN_REVISAR)
-- Revisión 0044-R2 — ver seguridad/44_revision_0044_r2.md para el detalle
-- completo de qué cambió respecto a la R1 y por qué, con las pruebas
-- empíricas que lo respaldan.
--
-- Origen: seguridad/41_post_review_hardening_plan.md — hardening derivado de
-- la revisión cruzada Kimi -> Claude (seguridad/40 y el hilo de
-- "REVISIÓN KIMI — HALLAZGOS VERIFICADOS"), más la revisión 0044-R2 sobre
-- la propia candidata (concurrencia, ACL, normalización, datos históricos).
--
...
--        de eventos draft/pending_review/hidden/rejected. Sin cambios en
--        R2 respecto a R1 (ver seguridad/44 §5 para la justificación de
--        mantener el denylist).
--   §2 — set_concern_listening_survey_response: p_community pasa de
--        validarse solo por longitud a normalizarse (trim + '' -> NULL) y
--        validarse contra el catálogo cerrado de 19 comunidades/ciudades
--        autónomas que usa el frontend (src/lib/data/regions.ts) — evita
--        que un valor inventado fuerce la supresión completa de
--        get_concern_listening_survey_territory_breakdown. R2 añade la
--        normalización previa a la validación (antes solo se normalizaba
--        al insertar, después de validar — un valor válido con espacios
--        incidentales se rechazaba).
...
--        real (nunca un valor enviado por el cliente). R2 añade: (a) un
--        advisory lock transaccional por (auth.uid(), action) para cerrar
--        una carrera real confirmada empíricamente (10 inserts concurrentes
--        del mismo usuario bypasseaban el límite de 3/min por completo
--        antes de este cambio — ver seguridad/44 §1); (b) REVOKE ALL sobre
--        write_rate_limits para PUBLIC/anon/authenticated (antes solo
--        dependía de RLS sin políticas, que ya bloqueaba en la práctica
--        pero dejaba una concesión de tabla completa sin usar como
--        superficie latente); (c) mensajes de límite neutrales, sin
--        prometer "mañana" para una ventana que en realidad es rolling.
--
-- NO se toca: is_moderator_or_admin (REFUTADA), atomicidad de delete-account
-- (REFUTADA como problema de seguridad — el mensaje de error se corrige
-- aparte, en supabase/functions/delete-account/index.ts, fuera de esta
-- migración porque no es SQL), votos/respuestas (ya protegidos por
-- UNIQUE+upsert), ni el diseño de set_attendance.
-- ============================================================================
...
--
-- Denylist elegido deliberadamente sobre allowlist (0044-R2 §5): es la
-- MISMA expresión, carácter a carácter, que ya usan events_select_public
-- (0003) y set_attendance (0014) para "visible públicamente" — la regla
-- canónica del proyecto. Un allowlist aquí divergería de esas dos y podría
-- desincronizarse silenciosamente si alguna cambia. El riesgo simétrico
-- (un estado nuevo que se añada sin actualizar las 3 apariciones quedaría
-- público por defecto) ya existe hoy en events_select_public/set_attendance
-- y no lo introduce este parche — se documenta como riesgo residual en
-- seguridad/44, con un control de baseline propuesto (§10) que lo detecta
-- si diverge.
...
-- ----------------------------------------------------------------------
-- Antes (R1): solo char_length(p_community) <= 120. get_concern_listening_
...
--
-- R2 corrige además un defecto de la propia R1: la normalización
-- (trim + '' -> NULL) ocurría solo al insertar, DESPUÉS de validar contra
-- el catálogo — un valor válido con espacios incidentales (' Cataluña ')
-- se rechazaba en vez de aceptarse, y '' se validaba como si fuera una
-- categoría inventada en vez de tratarse como "sin valor". Ahora la
-- normalización ocurre ANTES de la validación.
...
begin
	...
	-- NULL) ANTES de validar contra el catálogo cerrado (0044-R2 — mismo
	-- listado que src/lib/data/regions.ts, 19 comunidades/ciudades
	-- autónomas: 17 comunidades autónomas + Ceuta + Melilla; el comentario
	-- original de 0043 decía "20" por error de cuenta).
...
-- (0044-R2 — antes solo RLS-sin-políticas, que ya bloqueaba pero dejaba
-- una concesión de tabla completa sin usar).
...
-- alineados a ningún límite de reloj/calendario) — confirmado empíricamente
-- en 0044-R2. `concern_proposals` es más restrictivo porque proponer una
-- preocupación ciudadana es una acción deliberada y poco frecuente incluso
-- para un usuario muy comprometido; `reports`/`channel_reports` permiten
-- algo más de margen porque alguien puede legítimamente reportar varias
-- convocatorias o canales distintos en una sesión de uso normal.
...
begin
	...
	-- 0044-R2: serializa únicamente las transacciones concurrentes del
	-- MISMO usuario + MISMA acción. Sin esto, dos (o diez) inserts
	-- simultáneos del mismo usuario pueden contarse el uno al otro como
	-- "no comprometido todavía" bajo READ COMMITTED y saltarse el límite
	-- por completo — confirmado empíricamente: 10 reports concurrentes
	-- reales (pgbench, 10 conexiones ya establecidas, disparadas a la vez)
	-- -> 10 aceptados con un límite de ráfaga de 3/min, antes de este
	-- cambio. Ámbito de TRANSACCIÓN (pg_advisory_xact_lock, no de sesión):
	-- se libera solo al COMMIT/ROLLBACK de la propia inserción, sin
	-- desbloqueo manual. Claves independientes por usuario y por acción
	-- (dos enteros de 32 bits, hashtext de cada valor por separado, no
	-- concatenados) para que usuarios distintos y acciones distintas del
	-- mismo usuario nunca se bloqueen entre sí — no es un lock global.
	-- Probado empíricamente: usuarios distintos concurrentes, sin
	-- interferencia mutua; report+concern_proposal concurrentes del mismo
	-- usuario, sin bloqueo cruzado y cada límite se respeta por separado.
...
-- ============================================================================
-- Fin de 0044 (candidata, revisión R2). No aplicada a ningún entorno.
-- ============================================================================
