...
- Las 6 dependencias directas de producción se re-verificaron individualmente contra su propio `package.json`: `@fontsource-variable/inter` (OFL-1.1), `@fontsource-variable/space-grotesk` (OFL-1.1), `@lucide/svelte` (ISC), `@supabase/supabase-js` (MIT), `@vercel/og` (MPL-2.0), `maplibre-gl` (BSD-3-Clause). Todas permisivas, ninguna en conflicto con AGPL-3.0-only para el propio código de CONVOCA.
