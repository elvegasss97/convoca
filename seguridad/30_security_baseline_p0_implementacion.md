...
- **RLS demostrado contra una DB real, no contra texto:** `check-rls-cleanroom.mjs` usa `supabase@2.113.0 db diff --from <réplica vacía y desechable> --to migrations --schema public --output <archivo>`. Se eligió esta vía (en vez de aplicar las migraciones a mano vía `psql` contra un Postgres genérico) porque **se intentó primero la vía directa y falló**: la migración `0006` inserta en `storage.buckets`, tabla que solo existe si el bootstrap completo de plataforma (storage-api, gotrue) se ejecuta — exactamente lo que hace el shadow del CLI y lo que un contenedor Postgres genérico, arrancado a mano, no reproduce. Usar el propio CLI evita reimplementar ese bootstrap de forma no fiable. El diff se escribe directamente a un archivo (`--output`), nunca se captura como string gigante de stdout/stderr en el proceso Node — ver §3b para por qué esto importó de verdad, no solo en teoría. El diff resultante contiene `ALTER TABLE public.X ... ENABLE ROW LEVEL SECURITY` por cada tabla con RLS — cualquier tabla `CREATE TABLE public.X` sin esa línea emparejada es una tabla sin RLS. No asume 47 tablas: evalúa las que existan de verdad.

### 3b. Validación real en el propio PR (no solo en local)

Este workflow se ejecutó de verdad contra el PR #11 antes de considerarlo terminado — no se dio por bueno solo porque `pnpm security:baseline` pasara en local. La primera ejecución en GitHub Actions falló, y falló de formas que **nunca se reprodujeron en local**, confirmando que "pasa en mi máquina" no era suficiente evidencia. Se corrigió de forma iterativa, cada fix probado localmente antes de volver a empujar, hasta 7 iteraciones:

1. **`pnpm/action-setup@v4` sin versión** ("No pnpm version is specified") — `package.json` no declaraba `packageManager`. Fijado a `pnpm@11.9.0`.
2. **`db diff` devolvía texto plano en el runner, JSON en local** (mismo CLI, mismo comando) — el parseo basado en `--output-format json` no era fiable entre entornos. Primer intento de fix: fijar `--output-format json` explícitamente.
3. **`actions/setup-node@v5` fallaba buscando el binario `pnpm`** en jobs que no lo necesitaban, por la sola presencia de `packageManager` en `package.json` (auto-detección de corepack). Añadido `pnpm/action-setup@v4` a esos jobs también.
4. **El paso de RLS moría en silencio** (exit 1, sin ningún mensaje) a mitad de recibir el diff — hipótesis inicial: dos shadows completos de Supabase (el mío + el del CLI) agotando recursos en el mismo runner. Separado en jobs distintos (`migrations-cleanroom` / `migrations-rls`) para runners frescos — **no fue suficiente, seguía fallando igual, incluso aislado**.
5. Segunda hipótesis: la imagen completa de Supabase para mi propio destino vacío (que no necesita ninguna de sus extensiones) duplicaba innecesariamente memoria. Cambiado a `postgres:17-alpine` — **tampoco fue la causa completa**, mismo fallo.
6. **Causa real del "muere en silencio":** `console.log()` de un blob de ~80-200KB en una sola llamada. Confirmado indirectamente: al trocear el print en líneas cortas y recortarlo, el proceso dejó de morir y por primera vez se vio un mensaje de error real: "no se pudo encontrar/parsear el bloque JSON de diff".
7. **Causa real del fallo de parseo:** capturar el diff como JSON vía stdout/stderr no era fiable de extremo a extremo en el runner (contenido variable, probablemente de `npx` mezclado). Solución de fondo: `--output <archivo>` — el CLI escribe el diff como SQL plano directamente a un archivo; el script nunca vuelve a construir un string gigante ni a parsear JSON capturado. Verificado con 3 ejecuciones locales consecutivas (resultado idéntico, 47/47) y confirmado en GitHub Actions: **PASS completo, los 8 jobs relevantes en verde** (`resolve-base-ref`, `dependencies`, `migrations-structure`, `migrations-cleanroom`, `migrations-rls`, `code-static-checks`, `build-reproducible`, `secrets-diff`; `secrets-full-history` solo corre en `schedule`).

**Lección explícita, no solo anecdótica:** un check que pasa en local y nunca se ha visto pasar en el entorno real de CI no está terminado, por bien diseñado que parezca sobre el papel — el fallo #4-7 de esta lista era, en el fondo, un problema de robustez de la propia herramienta de verificación (cómo captura/imprime output grande), no del control de seguridad que pretendía implementar. Se documenta con el mismo detalle que el resto de este documento porque es exactamente el tipo de "lo probamos de verdad, no lo asumimos" que el resto de esta auditoría exige en todo lo demás.
