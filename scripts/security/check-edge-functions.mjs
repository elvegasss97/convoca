#!/usr/bin/env node
// G-SR — Edge Function nueva/modificada con credencial privilegiada de Supabase — seguridad/19 §4.
...
// (service_role, Secret API Key, createClient, auth.) — no por cualquier cambio de texto.
// Una función NUEVA que use una credencial privilegiada, o una NO inventariada
// que empiece a usarla, siempre dispara el gate.
...
const AUTH_RELATED = /service_role|SUPABASE_SERVICE_ROLE_KEY|SUPABASE_SECRET_KEYS|Secret API Key|createClient|\bauth\./i;
const PRIVILEGED_CREDENTIAL = /SUPABASE_SERVICE_ROLE_KEY|\bservice_role\b|SUPABASE_SECRET_KEYS|Secret API Key/i;

for (const path of changedFunctionFiles) {
	const content = getFileNow(path) ?? readFileSync(path, 'utf8');
	const usesPrivilegedCredential = PRIVILEGED_CREDENTIAL.test(content);
	if (!usesPrivilegedCredential) continue;
	...
	if (isOverridden('service-role', fnName)) {
		...
		failed = true;
		lines.push(
			`FAIL  GATE: ${path} usa una credencial privilegiada de Supabase (${isNew ? 'función nueva' : known ? 'cambio en línea de autorización de función ya inventariada' : 'función no inventariada'}). ` +
...
if (lines.length === 0) {
	lines.push('ok    ningún archivo modificado en supabase/functions/ usa credenciales privilegiadas de Supabase');
}
