...
<div>
	<div class="flex flex-wrap items-start justify-between gap-3">
		<div>
			<div class="flex items-center gap-2">
				...
				<h3 class="font-display text-base font-semibold text-ink-900">Radar municipal — por revisar</h3>
			</div>
			<p class="mt-1 max-w-3xl text-sm leading-relaxed text-ink-500">
				Nada de esta cola es público. Publicar exige revisión humana, una fuente y ubicación municipal
				canónica; la decisión queda auditada.
			</p>
		</div>
	...
	{#if items.length === 0}
		<div class="mt-4 rounded-2xl border border-dashed border-ink-200 bg-white px-5 py-10 text-center">
			...
			<p class="mt-1 text-xs text-ink-400">El radar puede seguir recopilando sin publicar automáticamente.</p>
		</div>
	{:else}
		<div class="mt-4 space-y-4">
			{#each items as item (item.id)}
				<article class="rounded-2xl border border-ink-100 bg-white p-5 shadow-sm">
						...
						<div class="min-w-0 flex-1">
							...
							<div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-ink-500">
								<span class="inline-flex items-center gap-1">
									<MapPin class="size-3.5" /> {item.municipalityName}
								</span>
								{#if item.municipalityIneCode}
									<span>INE {item.municipalityIneCode}</span>
								{:else}
									<span class="font-semibold text-critical-600">Sin código INE</span>
								{/if}
					...
					<p class="mt-3 text-sm leading-relaxed whitespace-pre-wrap text-ink-700">{item.summary}</p>
					...
					<div class="mt-4 grid gap-4 lg:grid-cols-2">
						<div class="rounded-xl bg-ink-50 p-3.5">
							...
							{#if item.sources.length === 0}
								<p class="mt-2 text-sm text-critical-600">Sin fuente: no se puede publicar.</p>
							{:else}
						...
						<div class="rounded-xl bg-ink-50 p-3.5">
							<p class="text-xs font-semibold tracking-wide text-ink-500 uppercase">Posibles vías</p>
					...
					<div class="mt-4 flex flex-wrap gap-2 border-t border-ink-100 pt-4">
						<button
							type="button"
							disabled={busyId === item.id || item.sources.length === 0 || !item.municipalityIneCode}
