	...
	<div class="mt-5 lg:flex lg:items-start lg:gap-6">
		...
		<div class="mt-4 min-w-0 flex-1 lg:mt-0">
				...
				<div class="space-y-8">
					...
					<section>
						...
						{#if attentionItems.length === 0}
							...
							>
								<CircleCheck class="size-5 shrink-0" /> Todo al día. No hay elementos pendientes de moderación
								en este momento.
							</div>
						{:else}
							<div class="mt-3 space-y-2">
								{#each attentionItems as item (item.key + item.label)}
									...
									>
										<span class="font-medium text-ink-800">{item.label}</span><ArrowRight
					...
					<section>
						...
						<div class="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
							<div class="rounded-2xl border border-ink-100 bg-white p-4">
								...
								{#if participantCount === null}<p class="mt-1 text-sm text-ink-400">
										Todavía sin datos
									</p>{:else}<p class="mt-1 text-sm text-ink-700">
							...
							<div class="rounded-2xl border border-ink-100 bg-white p-4">
								...
								{#if activeVoteRound}<p class="mt-1 text-sm text-ink-700">
									...
									<p class="mt-1 text-xs text-ink-500">
										{#if activeVoteTotal === null}Todavía sin datos de participación{:else}{activeVoteTotal}
											voto{activeVoteTotal === 1 ? '' : 's'} en total{/if}
									</p>{:else}<p class="mt-1 text-sm text-ink-400">
					...
					<section>
						...
						{#if recentAuditTrail === null}<p
								class="mt-3 flex items-center gap-1.5 text-sm text-ink-400"
							>
								<AlertCircle class="size-4" /> No se ha podido cargar la actividad reciente.
							</p>{:else if recentAuditTrail.length === 0}<p class="mt-3 text-sm text-ink-400">
								Todavía no hay actividad registrada.
							</p>{:else}<div class="mt-3 space-y-2">
								{#each recentAuditTrail as entry, i (i)}<div
									...
									>
										<div class="flex items-center gap-2 text-ink-700">
											<History class="size-4 shrink-0 text-ink-400" /><span
												><span class="font-medium text-ink-900"
													>{auditTrailActionLabels[entry.action] ?? entry.action}</span
												>
												· {auditTrailTargetTypeLabels[entry.targetType] ?? entry.targetType} · {auditTrailActorTypeLabels[
													entry.actorType
												]}</span
											>
										</div>
			...
			{:else if activeTab === 'pendientes'}
				{#if pending.length === 0}<p class="py-10 text-center text-sm text-ink-400">
						No hay convocatorias pendientes de revisión.
					</p>{:else}<div class="space-y-3">
						{#each pending as event (event.id)}<div
								class="rounded-2xl border border-ink-100 bg-white p-4"
							>
								<div class="flex flex-wrap items-start justify-between gap-2">
									<div>
										<div class="flex flex-wrap items-center gap-1.5">
											<StatusPill status={event.status} /><span
												class="rounded-full bg-ink-100 px-2.5 py-0.5 text-xs font-medium text-ink-600"
												>{categoryLabels[event.category]}</span
											>
										</div>
								...
								<div class="mt-3 flex flex-wrap gap-2 border-t border-ink-100 pt-3">
									<button
										...
										><Check class="size-3.5" /> Aprobar</button
									><button
										type="button"
										...
										><Pencil class="size-3.5" /> Solicitar cambios</button
									><button
										type="button"
										...
										><XIcon class="size-3.5" /> Rechazar</button
									><a
			...
			{:else if activeTab === 'reportadas'}
				{#if reported.length === 0}<p class="py-10 text-center text-sm text-ink-400">
						No hay convocatorias con reportes abiertos.
					</p>{:else}<div class="space-y-3">
						{#each reported as group (group.event.id)}<div
								class="border-critical-200 rounded-2xl border bg-critical-50/40 p-4"
							>
								<div class="flex flex-wrap items-center gap-1.5">
									<StatusPill status={group.event.status} /><VerificationBadge
										level={group.event.verification.level}
									/>
								</div>
								<p class="mt-1 font-display text-base font-semibold text-ink-900">
									{group.event.title}
								...
								<div class="mt-2 space-y-1.5">
									{#each group.reports as report (report.id)}<div
								...
								<div class="mt-3 flex flex-wrap gap-2 border-t border-critical-100 pt-3">
									<button
										...
										class="flex items-center gap-1.5 rounded-full bg-brand-700 px-3 py-1.5 text-xs font-semibold text-white hover:bg-brand-800"
										><Check class="size-3.5" /> Descartar reporte</button
									><button
										type="button"
										onclick={() => openAction(group.event, 'hide')}
										class="flex items-center gap-1.5 rounded-full border border-warning-300 px-3 py-1.5 text-xs font-semibold text-warning-700 hover:bg-warning-50"
										><EyeOff class="size-3.5" /> Ocultar</button
									><button
										type="button"
				...
				<div class="mt-6 border-t border-ink-100 pt-4">
					...
					{#if reportedChannels.length === 0}<p class="py-6 text-center text-sm text-ink-400">
							No hay canales con reportes abiertos.
						</p>{:else}<div class="mt-2 space-y-3">
							{#each reportedChannels as group (group.channel.id)}<div
									class="border-critical-200 rounded-2xl border bg-critical-50/40 p-4"
								>
									...
									<div class="mt-3 flex flex-wrap gap-2 border-t border-critical-100 pt-3">
										...
										><button
											...
											class="flex items-center gap-1.5 rounded-full border border-warning-300 px-3 py-1.5 text-xs font-semibold text-warning-700 hover:bg-warning-50"
											><EyeOff class="size-3.5" /> Ocultar solo el canal</button
										>
									</div>
								</div>{/each}
						</div>{/if}
				...
				<div class="mt-6 border-t border-ink-100 pt-4">
					<h3 class="font-display text-sm font-semibold text-ink-900">
						Firmas municipales reportadas
					</h3>
					<p class="mt-1 text-xs leading-relaxed text-ink-500">
						Publicar una recogida sigue siendo inmediato. Esta cola sirve únicamente para moderación
						posterior; la identidad de quien reportó no se muestra aquí.
					</p>
					{#if reportedMunicipalPetitions.length === 0}<p
							class="py-6 text-center text-sm text-ink-400"
						>
							No hay recogidas municipales con reportes abiertos.
						</p>{:else}<div class="mt-3 space-y-3">
							{#each reportedMunicipalPetitions as group (group.petition.id)}<div
									class="border-critical-200 rounded-2xl border bg-critical-50/40 p-4"
								>
									<div class="flex flex-wrap items-start justify-between gap-3">
										<div>
											<p
												class="inline-flex items-center gap-1 text-xs font-semibold text-brand-700"
											>
												<MapPin class="size-3.5" />
												{group.petition.municipalityName}
											</p>
											<p class="mt-1 font-display text-base font-semibold text-ink-900">
												{group.petition.title}
											</p>
											<p class="mt-1 text-xs leading-relaxed text-ink-500">
												{group.petition.requestText}
											</p>
										</div>
										<span
											class="rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-ink-600"
											>{group.petition.supportCount} apoyos</span
										>
									</div>
									<div class="mt-3 space-y-1.5">
										{#each group.reports as report (report.id)}<div
											...
											>
												<p class="text-xs font-semibold text-critical-700">
													{municipalReportReasonLabels[report.reason]}
												</p>
												{#if report.details}<p class="mt-0.5 text-xs text-ink-500">
														{report.details}
													</p>{/if}
												<div class="mt-2 flex flex-wrap gap-2">
													<button
														type="button"
														disabled={municipalReportActionId !== null}
														onclick={() => moderateMunicipalReport(report.id, 'dismiss')}
													...
													><button
														type="button"
														disabled={municipalReportActionId !== null}
														onclick={() => moderateMunicipalReport(report.id, 'hide_petition')}
														class="inline-flex items-center gap-1.5 rounded-full border border-warning-300 px-3 py-1.5 text-xs font-semibold text-warning-700 hover:bg-warning-50 disabled:opacity-50"
														><EyeOff class="size-3.5" /> Ocultar recogida</button
													>
												</div>
											</div>{/each}
									</div>
			...
			{:else if activeTab === 'documentacion'}
				{#if pendingDocuments.length === 0}<p class="py-10 text-center text-sm text-ink-400">
						No hay documentación pendiente de revisión.
					</p>{:else}<div class="space-y-2.5">
						{#each pendingDocuments as doc (doc.id)}<div
							...
							>
								...
								<div class="flex gap-2">
									<button
										...
										><FileSearch class="size-3.5" />{viewingDocId === doc.id
											? 'Abriendo…'
			...
			{:else if activeTab === 'gastoPublico'}
				<PublicSpendingModerationPanel
					bind:items={publicSpendingSubmissions}
					moderatorId={MODERATOR_ID}
				/>
			{:else if activeTab === 'temas'}
				<div>
					<a
						...
						><Plus class="size-4" /> Nuevo tema</a
					>{#if pendingAlternativesCount > 0}<p class="mt-3 text-sm text-warning-700">
						...
						</p>{/if}{#if topics.length > 0}<div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
							{#each topics as topic (topic.id)}<TopicCard
									{topic}
									showStatus
									href={`/moderacion/temas/${topic.id}`}
								/>{/each}
						</div>{:else}<p class="mt-4 py-10 text-center text-sm text-ink-400">
							Todavía no hay temas creados.
						</p>{/if}
				</div>
			{:else if activeTab === 'registro'}
				{#if auditLog.length === 0}<p class="py-10 text-center text-sm text-ink-400">
						Todavía no hay decisiones registradas.
					</p>{:else}<div class="space-y-2">
						{#each auditLog as log (log.id)}<div
								class="rounded-2xl border border-ink-100 bg-white px-4 py-3 text-sm"
							>
