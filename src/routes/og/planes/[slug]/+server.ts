...
import { getTopicBySlug } from '$lib/services/topicsService';
import { concernCategoryLabels } from '$lib/labels';

/**
 * Imagen social (1200×630) generada al vuelo para la previsualización al
 * compartir un plan de Pulso Ciudadano (WhatsApp/Telegram/X/Facebook/Discord).
 * Mismo patrón que `/og/convocatorias/[slug]` (ver ese archivo para el porqué
 * de cargar las fuentes desde Google Fonts en vez de un .woff2 local).
 */
...
async function fetchGoogleFont(family: string, weight: number): Promise<ArrayBuffer> {
	const cssUrl = `https://fonts.googleapis.com/css2?family=${family}&text=${encodeURIComponent(
		'PulsociudadanoPlanesdeConvocaCERCADETIabcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789áéíóúñÁÉÍÓÚÑ ,.·—€%'
	)}`;
...
export const GET: RequestHandler = async ({ params, fetch }) => {
	...
	if (!topic) error(404, 'Plan no encontrado');

	try {
		const fonts = await loadFonts();
		const subtitle = topic.investmentRange || topic.referenceGoal;
						...
						},
						{
							...
							props: {
								...
								children: topic.category
									? (concernCategoryLabels[topic.category] ?? 'Pulso ciudadano')
									: 'Pulso ciudadano'
							}
						},
						{
							...
							props: {
								...
								children: topic.documentTitle || topic.title
							}
						},
						...(subtitle
							? [
